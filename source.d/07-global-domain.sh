...
## New in Version 0.3
## if realm discover fails attempt realm list
...
if [ $BOL_FUNCTION -eq $TRUE ]; then
  ...
  if [ ${#DOMAIN}	-eq 0 ]; then declare -x DOMAIN=$($REALM_BIN list 2>/dev/null | $HEAD_BIN --lines=1);				fi
  if [ ${#DOMAIN}	-ne 0 ]; then
...
## Desired outcome:
# Populated Variables: $DOMAIN, $NB_DOMAIN, $REALM
