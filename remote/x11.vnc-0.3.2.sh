...
if [ $BOL_VERBOSE -eq $TRUE ]; then
  ...
  echo -e "Minimum Port:\t$VAR_MIN_PORT\nMaximum Port:\t$VAR_MAX_PORT\n"
fi
...
if [ $TASK_COUNT -eq 0 ]; then
	...
	if [ $DO_NOT_EXIT -eq $FALSE ]; then
		...
		if [ $BOL_WEBSOCKIFY -eq $TRUE ]; then
			sleep 3
			/usr/bin/websockify --daemon --ssl-only --web=/usr/share/novnc/ --cert=$WEBSOCK_CERT $(hostname -f):$((WEB_SOCK_PORT)) $(hostname -f):$((VNC_TCP_PORT))
		fi
	else
