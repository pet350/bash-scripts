...
declare -ig BOL_GET_NEXT_PORT=$TRUE
declare -ig BOL_CHECK_INSTANCE=$TRUE
...
declare -ig VAR_MAX_PORT=$((VAR_MIN_PORT+49))
...
if [ ${#XAUTHLOCALHOSTNAME} -eq 0 ]; then export XAUTHLOCALHOSTNAME="$(hostname -f)"; fi
...
function GetNextPort()
{
  ...
  while [ $BOL_LOOP -eq $TRUE ]; do
    ((COUNT++))
    nmap $(hostname -f) --open -p $((VAR_MIN_PORT))-$((VAR_MAX_PORT)) | grep $((COUNT+VAR_MIN_PORT)) >/dev/null
...
'--no-root-check')
        ...
        ;;
'--no-instance-check')
	export BOL_CHECK_INSTANCE=$FALSE
	;;
'--with-websockify')
...
--min-port=*)
        X="${i#*=}"
	export VAR_MIN_PORT=$((X))
	export VAR_MAX_PORT=$((VAR_MIN_PORT+49))
	;;
--port=*)
        X="${i#*=}"
	export BOL_GET_NEXT_PORT=$FALSE
	export VNC_TCP_PORT=$((X))
	;;
'-wl' | '--wait-longer')
...
'--no-exit')
        ...
        ;;
'--with-accept')
	export ACCEPT_OPT="-accept popup"
	;;
'--with-gone')
	export GONE_OPT="-gone popup"
	;;
*)
        (( UNKNOWN_COUNT++ ))
        ;;
...
# Define The Full Option Array
declare -ag VNC_OPT_ARRAY=("-rfbauth" "/home/$USERNAME/.vnc/passwd" "$ACCEPT_OPT"  "$GONE_OPT" "-ncache" \
...
if [ $BOL_VERBOSE -eq $TRUE ]; then
  ...
  if [ $BOL_GET_NEXT_PORT -eq $TRUE ]; then echo -e "Minimum Port:\t$VAR_MIN_PORT\nMaximum Port:\t$VAR_MAX_PORT\n"; fi
fi

if [ $BOL_GET_NEXT_PORT -eq $TRUE ]; then GetNextPort; fi
...
if [ $BOL_CHECK_INSTANCE -eq $FALSE ]; then TASK_COUNT=0; fi
...
if [ $TASK_COUNT -eq 0 ]; then
	...
	if [ $DO_NOT_EXIT -eq $FALSE ]; then
		...
		RETVAL=$?
		if [ $BOL_WEBSOCKIFY -eq $TRUE ]; then
			sleep 3
			/usr/bin/websockify --daemon --ssl-only --auto-pong --web=/usr/share/novnc/ --target-config=/usr/share/websockify/token.list --cert=$WEBSOCK_CERT $(hostname -f):$((WEB_SOCK_PORT)) $(hostname -f):$((VNC_TCP_PORT))
		fi
	else
