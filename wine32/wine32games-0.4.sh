...
{
  declare -i INDEX=-1
  for DATA in $(xrandr); do
    ((INDEX++))
    DATA_LEN=${#DATA}
    if [ $INDEX -eq 7 ]; then X_SIZE=$DATA;			fi
    if [ $INDEX -eq 9 ]; then Y_SIZE=${DATA:0:$((DATA_LEN-1))}; fi
  done
  export ORIGINAL_RES="$((X_SIZE))x$((Y_SIZE))"
  if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Current screen resolution: $ORIGINAL_RES"; fi
  return $SUCCESS
};
...
if [ $BOL_HELP		-eq $TRUE ]; then	do_HELP;	exit $FAILURE;		fi
if [ $VAR_UNKNOWN	-gt 0	  ]; then	exit $VAR_UNKNOWN;			fi
