...
{
   printf "%-10s\t%-10s\n\n" "$RUN_CMD:" "Version: $_VER"
   printf "%-10s\t" "Background:"; BOL_TEMP=$((BOL_BACKGROUND)); PrintBOL
   printf "%-10s\t" "Pause:"; BOL_TEMP=$((BOL_PAUSE_DOMAINS)); PrintBOL
   printf "%-10s\t" "UnPause:"; BOL_TEMP=$((BOL_UNPAUSE_DOMAINS)); PrintBOL
   printf "%-10s\t" "Status:"; BOL_TEMP=$((BOL_STATUS)); PrintBOL
   printf "%-10s\t" "Verbose:"; BOL_TEMP=$((BOL_VERBOSE)); PrintBOL
   printf "%-10s\t" "Test Mode:"; BOL_TEMP=$((BOL_TEST)); PrintBOL; printf "\n"
   return $SUCCESS
};
...
function doDiskImage()
{
   ...
   if [ $BOL_TEST -eq $FALSE ]; then
        if [ $BOL_VERBOSE -eq $TRUE ]; then
                echo -e "Executing: "$OPT_QUOTE"dd status=$OPT_STATUS if=$INFILE | bzip2 >$OUTFILE"$OPT_QUOTE"\n"
        fi
	...
	if [ $BOL_VERBOSE -eq $TRUE ]; then
		echo -e "Not Executing: "$OPT_QUOTE"dd status=$OPT_STATUS if=$INFILE | bzip2 >$OUTFILE"$OPT_QUOTE"\n"
	fi
	RETVAL=$FAILURE
   fi
   return $RETVAL
	...
	if [ ${BOL_DOMAIN_ARRAY_XL_LIST[$((USER_DATA_INDEX))]} -eq $TRUE ]; then
		if [ $BOL_VERBOSE -eq $TRUE ]; then
			echo -e "Pausing:\t$USER_DATA"
                ...
                if [ $BOL_TEST -eq $FALSE ]; then
                    if [ $BOL_VERBOSE -eq $TRUE ]; then
                        echo -e "Executing: "$OPT_QUOTE"/usr/sbin/xl pause $USER_DATA"$OPT_QUOTE"\n"
                    fi
                    /usr/sbin/xl pause $USER_DATA
                else
                    if [ $BOL_VERBOSE -eq $TRUE ]; then
                        echo -e "Not Executing: "$OPT_QUOTE"/usr/sbin/xl pause $USER_DATA"$OPT_QUOTE"\n"
                    fi
                fi
	fi
   done
...
function UnPauseRunningDomains()
{
   ...
   for USER_DATA in ${DOMAIN_ARRAY[@]}; do
		...
		if [ $BOL_TEST -eq $FALSE ]; then
		    if [ $BOL_VERBOSE -eq $TRUE ]; then
                        echo -e "Executing: "$OPT_QUOTE"/usr/sbin/xl unpause $USER_DATA"$OPT_QUOTE"\n"
                    fi
		    /usr/sbin/xl unpause $USER_DATA
		else
		    if [ $BOL_VERBOSE -eq $TRUE ]; then
			echo -e "Not Executing: "$OPT_QUOTE"/usr/sbin/xl unpause $USER_DATA"$OPT_QUOTE"\n"
		    fi
		fi
        fi
   done
