...
function CheckDestin()
{
  ...
  export LOGFILE="$LOG_PREFIX/$ARCHIVE_NAME.log"
...
function doBACKUP()
{
  ...
  CheckDestin
  AssembleOPTS
  echo -e "Backup Start Time: $(date)" >$LOGFILE
  if [ $BOL_REMOVE_OLD -eq $TRUE ]; then rm -vR $DESTINATION/$ARCHIVE_NAME* >>$LOGFILE; fi
  if [ $BOL_DO_BACKUP -eq $TRUE ]; then $RAR_BIN $OPTIONS $DESTINATION/$ARCHIVE_NAME $SOURCE_PATH >>$LOGFILE; fi
  RETVAL=$?
  if [ $BOL_DO_CHOWN -eq $TRUE ]; then $CHOWN_BIN -vR $ROOT_USER:$ROOT_GROUP $DESTINATION/$ARCHIVE_NAME* >>$LOGFILE; fi
  if [ $BOL_DO_CHMOD -eq $TRUE ]; then $CHMOD_BIN -vR $FILE_PERMISSIONS $DESTINATION/$ARCHIVE_NAME* >>$LOGFILE; fi
  if [ $BOL_DO_CHOWN -eq $TRUE ]; then $CHOWN_BIN -v $ROOT_USER:$ROOT_GROUP $LOGFILE >>$LOGFILE; fi
  if [ $BOL_DO_CHMOD -eq $TRUE ]; then $CHMOD_BIN -v $FILE_PERMISSIONS $LOGFILE >>$LOGFILE; fi
  echo -e "Backup End Time: $(date)" >>$LOGFILE
  return $RETVAL
};
