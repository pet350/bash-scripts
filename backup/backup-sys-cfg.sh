...
declare -ig BOL_CHECK_GZIP=$TRUE
...
# Define global array of all LXC virtual machines
declare -ag LXC_HOSTNAMES=("sql" "www" "ubuntuserver" "centos" "susesvr");

# Define What to backup
export backup_files="/etc /boot"

for DATA in ${LXC_HOSTNAMES[@]}; do
  if [ "$DATA" == "$LC_HOSTNAME" ]; then BOL_USR_LOCAL=$FALSE; fi
done

if [ $BOL_USR_LOCAL -eq $TRUE ]; then backup_files="$backup_files /usr/local"; fi
...
function PrintBOL()
{
   ...
   if [ $BOL_TEMP -eq $FALSE ];  then	printf "%-7s\n" "Disabled"
   elif [ $BOL_TEMP -eq $TRUE ]; then	printf "%-7s\n" "Enabled"
   else RETVAL=$FAILURE;		printf "%-7s\n" "Unknown"; fi
   return $RETVAL
};

# Check To Se If $RUN_PREFIX/$RUN_CMD Exists And Set BOL_Create_Symlink Accordingly
if [ ! -f $RUN_PREFIX/$RUN_CMD ]; then declare -ig BOL_Create_Symlink=$TRUE;	fi

# Check To Se If $CRON_FILE Exists And Set BOL_Create_Symlink Accordingly
if [ ! -f $CRON_FILE ]; then declare -ig BOL_Create_Cron_Job=$TRUE;		fi

# On Random Day Copy $backup_files to $uncompressed_dest
if [ $dow -eq $RAND_DOW ]; then declare -ig BOL_Copy_Uncompresses=$TRUE;	fi
...
function CHECK_GZIP_INSTANCES()
{
  declare -i GZIP_PID_COUNT=$(ps -ax | grep -v grep | grep gzip | wc -l)
  declare -i SECONDS=60

  while [ ${#GZIP_PID_COUNT} -gt 0 ]; do
    if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Detected GZIP Running. Waiting $SECONDS seconds."; fi
    sleep $SECONDS
    GZIP_PID_COUNT=$(ps -ax | grep -v grep | grep gzip | wc -l)
  done
}
...
for i in "$@"; do
  case $i in
    '--check-gzip')
	export BOL_CHECK_GZIP=$TRUE		;;
    '--skip-check-gzip')
	export BOL_CHECK_GZIP=$FALSE		;;
    '--create-cron')
...
export BACKUP_OPT="$BACKUP_OPT $VERBOSE"

# Execute appropriate commands based on booleans
if [ $BOL_HELP			-eq $TRUE	]; then		BOL_RUN=$FALSE;				BOL_CHECK_GZIP=$FALSE;		BOL_TAR_BACKUP=$FALSE;		BOL_TAR_BACKUP=$FALSE; 		do_HELP;				RETVAL=$FAILURE;	fi
if [ $VAR_UNKNOWN		-gt 0		]; then		BOL_RUN=$FALSE;				BOL_CHECK_GZIP=$FALSE;          BOL_TAR_BACKUP=$FALSE;          BOL_TAR_BACKUP=$FALSE;							RETVAL=$VAR_UNKNOWN;	fi
if [ $BOL_VERBOSE		-eq $TRUE	]; then		doVerboseHeader;																							fi
if [ $BOL_Make_Package_List	-eq $TRUE	]; then		$SCRIPT_PREFIX/$PACKAGE_SCRIPT;																						fi
if [ $BOL_Show_Random_Times	-eq $TRUE	]; then		SHOW_RANDOM_TIME;																							fi
if [ $BOL_Remove_Cron_Job	-eq $TRUE	]; then		rm -v $CRON_FILE;																							fi
if [ $BOL_Remove_Symlink	-eq $TRUE	]; then		rm -v $RUN_PREFIX/$RUN_CMD;																						fi
if [ $BOL_Create_Symlink	-eq $TRUE	]; then		cd "$RUN_PREFIX";			ln -fs $VERBOSE "$LN_PREFIX/$RUN_CMD";																fi
if [ $BOL_Create_Cron_Job	-eq $TRUE	]; then		CREATE_CRON_FILE;																							fi
if [ ! -d $uncompressed_dest 			]; then 	if [ $BOL_VERBOSE -eq $TRUE ];		then echo "Creating Directory $uncompressed_dest"; 					fi;	mkdir -p $uncompressed_dest;					fi
if [ $BOL_RUN			-eq $TRUE	]  &&		[ $BOL_CHECK_GZIP -eq $TRUE ];		then CHECK_GZIP_INSTANCES;																	fi
if [ $BOL_RUN			-eq $TRUE	]  && 		[ $BOL_TAR_BACKUP -eq $TRUE ];		then if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Creating: $dest/$archive_file";	fi;	tar czfv $dest/$archive_file $backup_files;			fi
if [ $BOL_RUN			-eq $TRUE 	]  &&		[ $BOL_Copy_Uncompresses -eq $TRUE ];	then if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Backing Up: $backup_files To: $uncompressed_dest"; fi; cp $BACKUP_OPT $backup_files $uncompressed_dest;	fi
if [ $BOL_RUN			-eq $TRUE	]; then		RETVAL=$?;																								fi
