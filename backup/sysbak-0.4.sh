...
declare -i PROC_COUNT=$(ps $PS_OPT | grep -v grep | grep $RUN_CMD | wc -l); ((PROC_COUNT--))
...
function UNMOUNT()
{
  ...
  if [ $BOL_QUIET -eq $FALSE ]; then printf "%s: " "$(SHOW_DATE_TIME)"; CLB_TEXT; printf "Results:\t"; LOG_RESULTS; unset COMMAND; echo -e '';															fi
  $SLEEP_BIN 1
  return $RETVAL
};
...
if [ ${#BAK_SOURCE_ARRAY[@]}			-eq 0 ]; then declare -ag BAK_SOURCE_ARRAY=( "$(SOURCE_PREFIX='/boot'; for SOURCE_FILE in efi extlinux flask grub grub2 init*-$KERNEL.img loafFINder System.map-$KERNEL config-$KERNEL vmlinuz-$KERNEL xen*; do VERIFY_SOURCE; done)" '.' '.' '.' "$(SOURCE_PREFIX='/usr/local'; for SOURCE_FILE in bin etc include lib lib64 libexec samba scripts sbin; do VERIFY_SOURCE; done)" );	fi
