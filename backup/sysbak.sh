...
declare -a CMD_ARRAY=( "zramctl" "modprobe" "lsmod" "grep" "scp" "rsync" );
...
export TEMP="mkfs.xfs";        export MKFS_XFS_BIN=$(GET_BIN)
export SQL_BACKUP="/usr/local/sbin/mysqlBackup.sh"
...
{
  declare -i RETVAL=$SUCCESS
  CC_TEXT; $RSYNC_BIN $VERBOSE -ur4 "$BAK_MOUNT/" "rsync://$NFS_SERVER/bak"; export RETVAL=$?; CN_TEXT
  export COMMAND="$RSYNC_BIN Return Value: $RETVAL"
...
# Added in Version 0.7; Usage of ZRAM Disk for Temporary Backup Location
function CREATE_ZRAMDRIVE()
{
  declare -i RETVAL=$SUCCESS

  if [ $BOL_ZCTL -eq $TRUE  ]; then
    $ZRAMCTL_BIN --size=$((ZRAM_SIZE))M $ZDEV
    export RETVAL=$?
    export COMMAND="$ZRAMCTL_BIN: Return Value: $RETVAL"
    if [ $BOL_QUIET -eq $FALSE ]; then printf "%s: " "$(SHOW_DATE_TIME)"; CLB_TEXT; printf "Results:\t"; LOG_RESULTS; unset COMMAND; printf "\n"; fi
  fi

  if [ $BOL_MKFS  -eq $TRUE  ]; then
    CC_TEXT; $MKFS_XFS_BIN $ZDEV; export RETVAL=$?; CN_TEXT;
    export COMMAND="$MKFS_XFS_BIN: Return Value: $RETVAL"
    if [ $BOL_QUIET -eq $FALSE ]; then printf "%s: " "$(SHOW_DATE_TIME)"; CLB_TEXT; printf "Results:\t"; LOG_RESULTS; unset COMMAND; printf "\n"; fi
  fi

  export NFS_EXPORT=$ZDEV
  export BOL_MNT=$TRUE
};

function CHECK_ZRAM_LOADED()
{
  declare -i RETVAL=$SUCCESS
  $LSMOD_BIN | $GREP_BIN $KMOD
  if [ $? -ne $SUCCESS ]; then
    if [ $BOL_VERBOSE -eq $TRUE ]; then printf "%s: " "$(SHOW_DATE_TIME)"; CLB_TEXT; printf "Kernel Module:\t"; CY_TEXT; printf "%s " $KMOD; CLR_TEXT; printf "Not loaded!\n";									fi
    if [ $BOL_QUIET  -eq $FALSE ]; then printf "%s: " "$(SHOW_DATE_TIME)"; CLB_TEXT; printf "Loading Module:\t"; CY_TEXT; printf "%s $s\n" $KMOD $KMOD_OPT;										fi
    $MODPROBE_BIN $KMOD $KMOD_OPT
    export RETVAL=$?
    export COMMAND="$MODPROBE_BIN: Return Value: $RETVAL"
    if [ $BOL_QUIET -eq $FALSE ]; then printf "%s: " "$(SHOW_DATE_TIME)"; CLB_TEXT; printf "Results:\t"; LOG_RESULTS; unset COMMAND; printf "\n";											fi
  fi
  return $RETVAL
};
...
for OPTIONS in $@; do
  case ${OPTIONS,,} in
    ...
    --no-mount)		export BOL_MNT=$FALSE;;
    --no-backup)	export BOL_BAK=$FALSE;;
    --no-mkfs)		export BOL_MKFS=$FALSE;;
    --no-zctl)		export BOL_ZCTL=$FALSE;;
    -s | --sql)		export BOL_SQL=$TRUE;;
    -z | --zram)	export BOL_ZRAM=$TRUE;;
...
if [ ${#BOL_ZRAM}						-eq 0 ]; then declare -i BOL_ZRAM=$FALSE;																fi
if [ ${#BOL_ZCTL}						-eq 0 ]; then declare -i BOL_ZCTL=$TRUE;																fi
if [ ${#BOL_MKFS}						-eq 0 ]; then declare -i BOL_MKFS=$TRUE;																fi
...
if [ ${#ZRAM_SIZE}						-eq 0 ]; then declare -i ZRAM_SIZE=1024;																fi
if [ ${#BOL_SQL}						-eq 0 ]; then declare -i BOL_SQL=$FALSE;																fi
if [ ${#ZDEV}							-eq 0 ]; then export ZDEV="/dev/zram5";																	fi
if [ ${#KMOD}							-eq 0 ]; then export KMOD="zram";																	fi
if [ ${#NUM_DEVICES}						-eq 0 ]; then export NUM_DEVICES=6;																	fi
if [ ${#KMOD_OPT}						-eq 0 ]; then export KMOD_OPT="num_devices=$NUM_DEVICES";														fi
...
if [ ${#DOW}							-eq 0 ]; then export DOW="$(date +%u)-$(date +%A)";															fi
...
if [ ${#SQL_BACKUP_OPTS}					-eq 0 ]; then export SQL_BACKUP_OPTS="$VERBOSE --no-mount --mysql-user=root --mysql-pass=Thund3rstruck!";														fi
if [ $BOL_ZRAM						    -eq $TRUE ]; then CHECK_ZRAM_LOADED; CREATE_ZRAMDRIVE;															fi
...
if [ ${#BAK_SOURCE_ARRAY[@]}					-eq 0 ]; then declare -ag BAK_SOURCE_ARRAY=( "$(SOURCE_PREFIX='/boot'; for SOURCE_FILE in efi extlinux flask grub grub2 initramfs-$KERNEL.img loafFINder System.map-$KERNEL config-$KERNEL vmlinuz-$KERNEL xen.gz xen-4.{10,11,12,13,14,15}.gz; do VERIFY_SOURCE; done)" '.' '.' '.' "$(SOURCE_PREFIX='/var'; for SOURCE_FILE in named www lib/httpd lib/apache2 lib/ipa-client lib/pki lib/tftpboot lib/tomcat lib/tomcat8 lib/tomcat9; do VERIFY_SOURCE; done)" "$(SOURCE_PREFIX='/usr/local'; for SOURCE_FILE in bin etc include lib lib64 libexec samba scripts sbin; do VERIFY_SOURCE; done)" ); fi																													##
if [ $BOL_BAK		 		    		    -eq $TRUE ]; then BACKUP; FINAL_RETVAL=$?;																	fi
if [ $BOL_SQL                                               -eq $TRUE ]; then $SQL_BACKUP $SQL_BACKUP_OPTS;                                                                          							fi
if [ $BOL_ZRAM						    -eq $TRUE ]; then SCP_ZRAM;																			fi
