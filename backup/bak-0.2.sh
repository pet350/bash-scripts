...
for DATA in xz curl egrep chown find true; do
  export TEMP="$DATA"
  TEMP_BIN=$(GET_BIN)
  if [ $? -eq $SUCCESS ]; then
...
function BACKUP()
{
    ...
    if [ -f $TARGET.xz ]; then
	declare -x CREATE="-u"
	INFO_EXEC_MESSAGE "$XZ_BIN --verbose --decompress $TARGET.xz"
	$XZ_BIN --verbose --decompress $TARGET.xz
	declare -i RETVAL=$?
	declare -x COMMAND=$XZ
	INFO_DONE_MESSAGE "Decompressing"
    else
	if [ -f $TARGET ]; then
	    declare -x CREATE="-u"
	else
    ...
    declare -x OPTIONS="$CREATE -vf"
    INFO_EXEC_MESSAGE "$TAR_BIN $OPTIONS $TARGET $SNAP $SOURCE"
    $TAR_BIN $OPTIONS $TARGET $SNAP $SOURCE
    declare -i RETVAL=$?
    declare -x COMMAND=$TAR_BIN
    INFO_DONE_MESSAGE "tar backup"
    return $RETVAL
};
...
for OPTIONS in $@; do
	...
	--test)			declare -i BOL_TEST=$TRUE;			declare -x XZ_BIN=$TRUE_BIN;		declare -x TAR_BIN=$TRUE_BIN;;
...
if [ ${#SOURCE_LIST[@]}		-eq 0 ]; then declare -a SOURCE_LIST=(		"$(CHECK_PREFIX /etc)"			"$(CHECK_PREFIX /boot)"					\
...
if [ ${#DOW_TXT}		-eq 0 ]; then declare -x DOW_TXT=$(date +%A);					fi
...
if [ ${#BAK_FILENAME}		-eq 0 ]; then declare -x BAK_FILENAME="$DOW_TXT.backup";			fi
if [ ${#SNAP_FILENAME}		-eq 0 ]; then declare -x SNAP_FILENAME="$DOW_TXT.backup";			fi
if [ ${#COMPRESS}		-eq 0 ]; then declare -x COMPRESS="";						fi

# Reassemble list without all the spaces
for TEMP in ${SOURCE_LIST[@]}; do
    TEMP_LIST="$TEMP_LIST $TEMP\n"
done

declare -x BACKUP_LIST="$TEMP_LIST"
declare -x BACKUP_TARGET="$NFS_PREFIX/$NFS_SERVER/$NFS_EXPORT/$HOSTNAME/$DOW"
if [ ! -d "$BACKUP_TARGET" ]; then mkdir -p -v "$BACKUP_TARGET";							fi
...
while IFS= read LINE; do
    ((INDEX++))
    declare -x ARCHIVE="$BACKUP_TARGET/$BAK_FILENAME.$INDEX.tar"
    declare -x SNAPSHOT="$BACKUP_TARGET/$SNAP_FILENAME.$INDEX.snap"

    if [ ${#LINE} -gt 1 ]; then
        if [ -d $LINE ] || [ -f $LINE ]; then
            BACKUP $LINE $ARCHIVE $SNAPSHOT
            RETVAL=$?
	fi
    fi
    if [ $RETVAL -eq $SUCCESS ]; then
        INFO_EXEC_MESSAGE "$XZ_BIN --verbose --compress $ARCHIVE"
        $XZ_BIN --verbose --compress $ARCHIVE
        declare -i RETVAL=$?
        declare -x COMMAND=$XZ_BIN
        INFO_DONE_MESSAGE "Compressing"
    fi
done < <(echo -e "$BACKUP_LIST")
