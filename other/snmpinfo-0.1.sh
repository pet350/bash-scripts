...
if [ ${#SNMP_OPTIONS} -ne 0 ] && [ ${#SNMP_COMMUNITY} -ne 0 ] && [ ${#SNMP_SERVER} -ne 0 ] && [ $BOL_REQUIRED -eq $TRUE ]; then
	...
	if [ $ENABLE -eq $TRUE ] || [ $BOL_ALL -eq $TRUE ]; then
	    ...
	    if [ ${#DATA_LEN} -eq 0 ]; then declare -i DATA_LEN=0; fi
            ...
            done < <($SNMPWALK_BIN $SNMP_OPTIONS -c$SNMP_COMMUNITY  $SNMP_SERVER $ITEM; declare -i RETVAL=$?)
	fi
      done
    else
	...
	DEBUG_START_MESSAGE "Loop Through snmpwalk" >/dev/stderr
	DEBUG_EXEC_MESSAGE "$SNMPWALK_BIN $SNMP_OPTIONS -c$SNMP_COMMUNITY  $SNMP_SERVER $ITEM" >/dev/stderr
    fi
    for ENABLE in ${ENABLE_ARRAY[@]}; do
        ((INDEX++))
        if [ $ENABLE -eq $TRUE ] || [ $BOL_ALL -eq $TRUE ]; then
