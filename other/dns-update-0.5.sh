...
export MODIFIED="2022-03-30, 2022-06-19, 2022-11-19"
...
function DEL_RECORD()
{
  ...
  if [ ${#A_RECORD} -ne 0 ]; then
    ...
    for LOOP_IP_ADDRESS in ${HOST_RECORD_ADDRESS_ARRAY[@]}; do										 # Changed in Ver 0.2
      ...
      if [ $BOL_DEBUG -eq $TRUE ]; then PRINT_DATE_TIME;echo "[Debug] $ST_BIN dns delete $SERVER $ZONE ${HOST_NAME%%.*} A $LOOP_IP_ADDRESS --username=$UPDATE_USER --password=$UPDATE_PASS"; fi
...
if [ ${#SERVER}		-eq 0	]; then export SERVER="kdc.gigaware.lan";			fi
