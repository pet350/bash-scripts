...
# Define Booleans only if not already set by environment
...
if [ ${#REMOTE_HOST}	-eq 0 ]; then
  case ${LOCAL_HOST,,} in
    "lxc.gigaware.lan") export REMOTE_HOST="ipa.gigaware.lan";;
    *)			export REMOTE_HOST="lxc.gigaware.lan";;
  esac
fi

# Define the Log Filename
if [ ${#DOW}		-eq 0 ]; then export DOW=$(date +%w);						fi
if [ ${#FULL_DATE}	-eq 0 ]; then export FULL_DATE=$(date +%F);					fi
if [ ${#FULL_TIME}	-eq 0 ]; then export FULL_TIME=$(date +%r);					fi
if [ ${#LOGFILE}	-eq 0 ]; then export LOGFILE="/var/log/unison/$DOW-Server.log";			fi
if [ ${#BACKUP_PATH}	-eq 0 ]; then export BACKUP_PATH="/opt/bak/scripts/$(hostname -s)";		fi
...
for i in "$@"; do
  case $i in
    ...
    -V | --version)	echo -e "$RUN_CMD\tVersion: $VERSION\nBy: Peter Talbott";		exit $SUCCESS;;
    -b | --bw)		export BOL_COLOR=$FALSE;;
    -c | --color)	export BOL_COLOR=$TRUE;;
    ...
    --threshold=*)	export X="${i#*=}";			export PROC_THRESHOLD=$((X));;
    --remote=*)		export REMOTE_HOST="${i#*=}";;
    -w=* | --wait=*)	export X="${i#*=}"; 			export VAR_WAIT=$((X));				export BOL_WAIT=$TRUE;;
    --custom-folder=*)  export CUSTOM_PREFIX="${i#*=}";		export CUSTOM_FOLDER="${CUSTOM_PREFIX#*/}";	export BOL_CUSTOM_FOLDER=$TRUE;		export INDEX=${#FOLDER1_ARRAY[@]};		FOLDER1_ARRAY[$((INDEX))]="$CUSTOM_PREFIX";			FOLDER2_ARRAY[$((INDEX))]="/nfs/$REMOTE_HOST/$CUSTOM_FOLDER";;
    --add-folder=*)	export CUSTOM_PREFIX="${i#*=}";		export CUSTOM_FOLDER="${CUSTOM_PREFIX#*/}";	export INDEX=${#FOLDER1_ARRAY[@]};	FOLDER1_ARRAY[$((INDEX))]="$CUSTOM_PREFIX"	FOLDER2_ARRAY[$((INDEX))]="/nfs/$REMOTE_HOST/$CUSTOM_FOLDER";;
    *)
