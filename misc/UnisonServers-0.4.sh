...
if [ $CURRENT_PROC -gt $PROC_THRESHOLD ]; then
  if [ $BOL_QUIET -eq $TRUE ]; then
    echo -e $COLOR_YELLOW"[$FULL_DATE @ $FULL_TIME]"$COLOR_LT_BLUE"Current Process Count: "$COLOR_YELLOW"$CURRENT_PROC "$COLOR_LT_BLUE"Excedes Threshold: "$COLOR_YELLOW"$PROC_THRESHOLD!"$COLOR_NORMAL | tee -a "$LOGFILE.ERROR"
  else
    echo -e $COLOR_LT_GREEN"$RUN_CMD"$COLOR_LT_BLUE"\tVersion: "$COLOR_YELLOW"$VERSION"$COLOR_NORMAL | tee -a "$LOGFILE.ERROR"
    echo -e $COLOR_LT_GREEN"Start of Log: "$COLOR_YELLOW"[$FULL_DATE @ $FULL_TIME]"$COLOR_NORMAL | tee -a "$LOGFILE.ERROR"
...
if [ $BOL_QUIET -ne $TRUE ]; then echo -e $COLOR_LT_GREEN"$RUN_CMD"$COLOR_LT_BLUE"\tVersion: "$COLOR_YELLOW"$VERSION"$COLOR_NORMAL | tee -a $LOGFILE; fi
echo -e $COLOR_LT_GREEN"Start of Log: "$COLOR_YELLOW"[$FULL_DATE @ $FULL_TIME]"$COLOR_NORMAL | tee -a $LOGFILE
if [ $BOL_QUIET -ne $TRUE ]; then echo -e $COLOR_LT_GREEN"Local Host Name: "$COLOR_YELLOW"$LOCAL_HOST"$COLOR_NORMAL | tee -a $LOGFILE; fi
if [ $BOL_QUIET -ne $TRUE ]; then echo -e $COLOR_LT_GREEN"Remote Host Name: "$COLOR_YELLOW"$REMOTE_HOST"$COLOR_NORMAL | tee -a $LOGFILE; fi
if [ $BOL_QUIET -ne $TRUE ]; then echo -e $COLOR_LT_GREEN"Number of Running Unison Processess: "$COLOR_YELLOW"$CURRENT_PROC"$COLOR_NORMAL | tee -a $LOGFILE; fi
if [ $BOL_QUIET -ne $TRUE ]; then echo -e $COLOR_LT_GREEN"Threshold of Running Unison Processess: "$COLOR_YELLOW"$PROC_THRESHOLD"$COLOR_NORMAL | tee -a $LOGFILE; fi
...
if [ $BOL_QUIET -ne $TRUE ]; then
  echo -e $COLOR_LT_GREEN"Local Folder List: "$COLOR_YELLOW"$FOLDER1_LIST"$COLOR_NORMAL | tee -a $LOGFILE
  echo -e $COLOR_LT_GREEN"Remote Folder List: "$COLOR_YELLOW"$FOLDER2_LIST"$COLOR_NORMAL | tee -a $LOGFILE
  echo -e $COLOR_LT_GREEN"Unison Options: "$COLOR_YELLOW"${OPTIONS_ARRAY[@]}"$COLOR_NORMAL | tee -a $LOGFILE
  echo -e $COLOR_LT_GREEN"Log File: "$COLOR_YELLOW"$LOGFILE"$COLOR_NORMAL | tee -a $LOGFILE
fi
...
for DATA in ${FOLDER1_ARRAY[@]}; do
  ...
  echo -e $COLOR_LT_BLUE"Executing: "$COLOR_YELLOW"$COMMAND $FOLDER1 $FOLDER2 ${OPTIONS_ARRAY[@]} \
	-ignore 'Name .cache*' -ignore 'Name plasmaConfSaver*' -ignore 'Name icons*' -prefer $FOLDER_PREFERENCE -nodeletion $FOLDER1 -logfile $LOGFILE.INFO | tee -a $LOGFILE"
