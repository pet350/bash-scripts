...
for OPTIONS in $@; do
	...
	--no-men | -m)	for LIST in ${GUYS[@]}; do if [ ${#EXCLUDE_LIST} -eq 0 ]; then export EXCLUDE_LIST="$LIST";		else export EXCLUDE_LIST="$EXCLUDE_LIST|$LIST";		fi;	done;;
    esac
done
...
function DL()
{
    ...
    for SUFFIX in ${LIST_ARRAY[@]}; do
		...
		if [ $BOL_VERBOSE -eq $TRUE ]; then CLB_TEXT; printf "Target file: "; CY_TEXT; printf "%s/%s\n" $TARGET_PREFIX $TARGET_FILENAME; CN_TEXT;			fi
		if [ ! -f "$TARGET_PREFIX"/"$TARGET_FILENAME" ]; then
			...
			LOG_RESULTS
			if [ $RETVAL -ne $SUCCESS ]; then ((TOTAL_ERRORS++)); fi
			if [ $RETVAL -eq $SUCCESS ]; then ((SUCCESSFUL_DOWNLOAD++)); fi 
			echo -e "\n"
			CHANGE_OWNER
			MOVE_TARGET
		else
