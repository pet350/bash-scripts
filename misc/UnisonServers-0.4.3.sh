...
export BACKUP_FILE="/opt/bak/scripts/$(hostname -s)/scripts-$FULL_DATE.tar.gz"
...
function BACKUP_SCRIPTS()
{
  ...
  $TAR_BIN $BACKUP_OPTIONS $BACKUP_FILE -C "/usr/local" "scripts" | tee -a $LOGFILE
  export RETVAL=$?
...
for DATA in ${FOLDER1_ARRAY[@]}; do
  ...
  echo -e $COLOR_LT_BLUE"Executing: "$COLOR_YELLOW"$COMMAND $FOLDER1 $FOLDER2 ${OPTIONS_ARRAY[@]} \
	-ignore 'Name .cache*' -ignore 'Name plasmaConfSaver*' -ignore 'Name icons*' -prefer $FOLDER_PREFERENCE -nodeletion $FOLDER_PREFERENCE -logfile $LOGFILE.INFO | tee -a $LOGFILE"
  PRINT_DATE_TIME
