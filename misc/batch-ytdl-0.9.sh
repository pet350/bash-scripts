...
for OPTIONS in $@; do
	...
	--no-men |	-m)		NO_MEN;;
        *)				declare -x ADD_OPT="$ADD_OPT $OPTIONS";;
    esac
done
...
function SHOW_DL()
{
    ...
    if [ ${#ADD_OPT} -ne 0 ]; then
      CLB_TEXT; printf "Added Opts: ";      CY_TEXT; printf "%s\n" $ADD_OPT; CN_TEXT
    fi 
    return $SUCCESS
};
...
function DL()
{
    ...
    for SUFFIX in ${LIST_ARRAY[@]}; do
		...
		if [ ! -f "$TARGET_PREFIX"/"$TARGET_FILENAME" ]; then
			...
			if [ $BOL_DEBUG   -eq $TRUE ]; then CLR_TEXT; printf "[Debug] "; CLB_TEXT; printf "Executing: "; CY_TEXT; printf "%s %s %s %s\n" $YT_DLP_BIN $DLOPTS $VERBOSE $ADD_OPT $FULL_URL; CN_TEXT;             fi
			CC_TEXT; $YT_DLP_BIN $DLOPTS $VERBOSE $ADD_OPT $FULL_URL
