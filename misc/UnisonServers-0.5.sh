...
if [ ${#CFG_FILE}		-eq 0 ]; then export CFG_FILE="/etc/unisonservers.cfg";	fi
...
if [ ${#BOL_HOME}		-eq 0 ]; then declare -ig BOL_HOME=$TRUE;		fi
if [ ${#BOL_USR_LOCAL}		-eq 0 ]; then declare -ig BOL_USR_LOCAL=$TRUE;		fi
if [ ${#BOL_CUSTOM_FOLDER}	-eq 0 ]; then declare -ig BOL_CUSTOM_FOLDER=$FALSE;	fi
if [ ${#BOL_QUIET}		-eq 0 ]; then declare -ig BOL_QUIET=$FALSE;		fi
if [ ${#BOL_REQ_ROOT}		-eq 0 ]; then declare -ig BOL_PREFER_REMOTE=$FALSE;	fi
if [ ${#BOL_ALLOW_DELETE}	-eq 0 ]; then declare -ig BOL_ALLOW_DELETE=$FALSE;	fi
if [ ${#BOL_NO_PREFERANCE}	-eq 0 ]; then declare -ig BOL_NO_PREFERANCE=$FALSE;	fi
if [ ${#BOL_REQ_ROOT}		-eq 0 ]; then declare -ig BOL_REQ_ROOT=$TRUE;		fi
if [ ${#BOL_BACKUP}		-eq 0 ]; then declare -ig BOL_BACKUP=$FALSE;		fi
if [ ${#BOL_KRB5}		-eq 0 ]; then declare -ig BOL_KRB5=$FALSE;		fi
if [ ${#BOL_LOG}		-eq 0 ]; then declare -ig BOL_LOG=$TRUE;		fi

# Define String Variables
if [ ${#LOG_PREFIX}		-eq 0 ]; then export LOG_PREFIX="/var/log/unison";	fi
if [ ${#LOCAL_HOST}		-eq 0 ]; then export LOCAL_HOST="$(hostname -f)";	fi
if [ ${#COMMAND}		-eq 0 ]; then export COMMAND="$UNISON_BIN";		fi

# Define Integers
if [ ${#VAR_WAIT}		-eq 0 ]; then declare -ig VAR_WAIT=1;			fi
if [ ${#PROC_THRESHOLD}		-eq 0 ]; then declare -ig PROC_THRESHOLD=1;		fi

# Set Remote Hostname based on Local Hostname
if [ ${#REMOTE_HOST}		-eq 0 ]; then
  if [ $LOCAL_HOST == "lxc.gigaware.lan" ]; then
    export REMOTE_HOST="dc.gigaware.lan"
  else
...
if [ ${#LOGFILE}		-eq 0 ]; then export LOGFILE="/var/log/unison/$DOW-Server.log";			fi
if [ ${#BACKUP_PATH}		-eq 0 ]; then export BACKUP_PATH="/opt/bak/$(hostname --fqdn)/scripts";		fi
if [ ${#BACKUP_FILE}		-eq 0 ]; then export BACKUP_FILE="$BACKUP_PATH/scripts-$FULL_DATE.tar.gz";	fi
if [ ${#BACKUP_OPTIONS}		-eq 0 ]; then export BACKUP_OPTIONS="-zcvf";					fi
if [ ${#INFO}			-eq 0 ]; then export INFO=".INFO";						fi
...
for OPTIONS in "$@"; do
  case ${OPTIONS,,} in
    --krb5)			export BOL_KRB5=$TRUE;;
    --no-wait)			export BOL_WAIT=$FALSE;;
    --non-root)			export BOL_REQ_ROOT=$FALSE;;
    --backup)			export BOL_BACKUP=$TRUE;;
    --allow-delete)		export BOL_ALLOW_DELETE=$TRUE;;
    --no-preferance)		export BOL_NO_PREFERANCE=$TRUE;;
    --no-log)			export BOL_LOG=$FALSE;;
    --bw)			export BOL_COLOR=$FALSE;;
    --color)			export BOL_COLOR=$TRUE;;
    --force-color)		export BOL_FORCE_COLOR=$TRUE;	export BOL_COLOR=$TRUE;;
    --no-home)			export BOL_HOME=$FALSE;;
    --with-home)		export BOL_HOME=$TRUE;;
    --no-usr-local)		export BOL_USR_LOCAL=$FALSE;;
    --with-usr-local)		export BOL_USR_LOCAL=$TRUE;;
    --prefer-local)		export BOL_PREFER_REMOTE=$FALSE;;
    --prefer-remote)		export BOL_PREFER_REMOTE=$TRUE;;
    -h | --help)		export BOL_HELP=$TRUE;		export VERBOSE="";		export BOL_DEBUG=$FALSE;		export BOL_VERBOSE=$FALSE;		export BOL_LOG_RESULTS=$FALSE;;
    -t | --test)		export UNISON_BIN="$TRUE_BIN";	export TAR_BIN="$TRUE_BIN";	export COMMAND="$TRUE_BIN";		export BOL_TEST=$TRUE;;
    -d | --debug)		export VERBOSE="--verbose";	export BOL_DEBUG=$TRUE;		export BOL_VERBOSE=$TRUE;		export BOL_LOG_RESULTS=$TRUE;		export BOL_QUIET=$FALSE;;
    -v | --verbose)		export VERBOSE="--verbose";	export BOL_VERBOSE=$TRUE;	export BOL_LOG_RESULTS=$TRUE;		export BOL_QUIET=$FALSE;;
    -q | --quiet)		export VERBOSE="";		export BOL_VERBOSE=$FALSE;	export BOL_LOG_RESULTS=$FALSE;		export BOL_QUIET=$TRUE;;
    --version)			echo -e "$RUN_CMD\tVersion: $VERSION\nBy: Peter Talbott";	exit $SUCCESS;;
    --threshold=*)		export X="${OPTIONS#*=}";	export PROC_THRESHOLD=$((X));;
    --remote=*)			export REMOTE_HOST="${OPTIONS#*=}";;
    -w=* | --wait=*)		export X="${OPTIONS#*=}";	VAR_WAIT=$((X));		export BOL_WAIT=$TRUE;;
    --custom-folder=*)		export CUSTOM_PREFIX="${OPTIONS#*=}";	export CUSTOM_FOLDER="${CUSTOM_PREFIX#*/}";	export BOL_CUSTOM_FOLDER=$TRUE;		export INDEX=${#FOLDER1_ARRAY[@]}
				FOLDER1_ARRAY[$((INDEX))]="$CUSTOM_PREFIX";	FOLDER2_ARRAY[$((INDEX))]="/nfs/$REMOTE_HOST/$CUSTOM_FOLDER";;
    --add-folder=*)		export CUSTOM_PREFIX="${OPTIONS#*=}";	export CUSTOM_FOLDER="${CUSTOM_PREFIX#*/}";	export BOL_CUSTOM_FOLDER=$FALSE;     	export INDEX=${#FOLDER1_ARRAY[@]}
				FOLDER1_ARRAY[$((INDEX))]="$CUSTOM_PREFIX";	FOLDER2_ARRAY[$((INDEX))]="/nfs/$REMOTE_HOST/$CUSTOM_FOLDER";;
    *)				export OPTIONS_ARRAY_COUNT=${#OPTIONS_ARRAY[@]};					OPTIONS_ARRAY[$((OPTIONS_ARRAY_COUNT))]="$OPTIONS";;
  esac
done

if [ $BOL_CUSTOM_FOLDER -eq $TRUE ]; then export BOL_USR_LOCAL=$FALSE; export BOL_HOME=$FALSE;	fi
if [ $BOL_COLOR		-eq $TRUE ]; then INIT_COLOR;						fi
if [ $BOL_HELP		-eq $TRUE ]; then DO_HELP;						fi
if [ $BOL_REQ_ROOT	-eq $TRUE ]; then REQUIRE_ROOT_USER;					fi
if [ $BOL_KRB5		-eq $TRUE ]; then /usr/local/sbin/q5;					fi
if [ $BOL_LOG		-ne $TRUE ]; then LOGFILE="/dev/null"; INFO="";				fi
if [ $BOL_LOG		-eq $TRUE ] && [ ! -d $LOG_PREFIX ]; then mkdir -p $LOG_PREFIX;		fi
...
if [ $BOL_QUIET -ne $TRUE ]; then PRINT_DATE_TIME; echo -e $COLOR_LT_GREEN"User Info: "$COLOR_YELLOW"$(id)"$COLOR_NORMAL | tee -a $LOGFILE; fi
if [ $BOL_QUIET -ne $TRUE ]; then PRINT_DATE_TIME; echo -e $COLOR_LT_GREEN"Log File: "$COLOR_YELLOW"$LOGFILE"$COLOR_NORMAL | tee -a $LOGFILE; fi
