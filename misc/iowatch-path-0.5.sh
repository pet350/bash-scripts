...
function MONITOR_PATH()
{
  ...
  if [ $? -eq $SUCCESS ]; then
    ...
    if [ $BOL_DEBUG -eq $TRUE ]; then echo -e "Executing: $UNISON_SCRIPT --custom-folder=$UNISON_PATH $ALLOW_USER_STRING --threshold=1 $DELETE --no-preferance $COLOR_STRING $TEST_STRING $VERBOSE_STRING $DEBUG_STRING ${OPT_ARRAY[@]}"; fi
    $UNISON_SCRIPT --custom-folder="$UNISON_PATH" $ALLOW_USER_STRING --threshold=1 $DELETE --no-preferance $COLOR_STRING $TEST_STRING $VERBOSE_STRING $DEBUG_STRING ${OPT_ARRAY[@]} | tee -a "$LOGFILE"
    FUNCTION_RETURN=$?
  fi
...
case $(hostname -f) in
  'lxc.gigaware.lan')		export DELETE='';;
