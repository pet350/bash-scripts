...
# This binary is used to get the TARGET_FILENAME Only!
...
function GET_LIST_ARRAY()
{
    ...
    while IFS= read LINE; do
		...
		*)
			((LIST_INDEX++))
			if [ $BOL_LENGTH -eq $TRUE ]; then TEMP_LENGTH=$($YT_DLP_BIN $LENGTH_OPT $BASE_URL$TEMP); else TEMP_LENGTH="Unknown";	fi
...
function DL()
{
    ...
    for SUFFIX in ${LIST_ARRAY[@]}; do
		...
		if [ ! -f "$TARGET_PREFIX"/"$TARGET_FILENAME" ]; then
			...
			RETVAL=$?; COMMAND="$YT_DLP_BIN"
			LOG_RESULTS
			echo -e "\n"
			CHANGE_OWNER
			MOVE_TARGET
		else
			printf "Target Exists! Skipping Download!\n\n"
		fi
        fi
    done
};
