...
for FILE in $(find $FOLDER1 -name '*(copy:*'); do rm -v $FILE; done | tee >>$LOGFILE.OUT >/dev/stdout

# Run Unison With The PreDefined Variables Above
unison $FOLDER1 $FOLDER2 $OPTIONS -prefer $FOLDER1 -nodeletion $FOLDER1 -logfile $LOGFILE | tee >>$LOGFILE.OUT >/dev/stdout
