...
function FFMPEG_ENCODE()
{
  ...
  if [ $BOL_DEBUG -eq $TRUE ]; then echo -e "Executing: $FFMPEG_BIN -i $INPUT_FILES ${FFMPEG_OPT[@]} $OUTPUT_FILE"; fi
  $FFMPEG_BIN -i "$INPUT_FILES" ${FFMPEG_OPT[@]} "$OUTPUT_FILE"
  return $?
};
...
*)
	...
	if [ $BOL_FFMPEG -eq $FALSE ]; then
	  echo -e "$RUN_CMD\tUnknown Option: $i\n\n"
	else
	  echo -e "$RUN_CMD\tAppended $i to FFMPEG Options\n"
	fi
	;;
esac
done
