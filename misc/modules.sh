...
function MODPROBE_MODULES()
{
  ...
  while IFS= read LINE; do
    if [ ${#LINE} -ne 0 ]; then
      export COMMAND="$MODPROBE_BIN $REMOVE $VERBOSE $LINE"
      $MODPROBE_BIN $REMOVE $VERBOSE $LINE; export RETVAL=$?
...
for OPTIONS in $@; do
  case ${OPTIONS,,} in
    ...
    'start')            declare -i BOL_START=$TRUE;     declare -i BOL_STOP=$FALSE;	export REMOVE='';;
    'stop')             declare -i BOL_START=$FALSE;    declare -i BOL_STOP=$TRUE;	export REMOVE='-r';;
    'restart')          declare -i BOL_START=$TRUE;     declare -i BOL_STOP=$TRUE;	export REMOVE='-r';;
...
if [ $BOL_STOP  -eq $TRUE ] && [ -f $MODFILE ]; then MODPROBE_MODULES;						fi
if [ $BOL_START	-eq $TRUE ] && [ -f $MODFILE ]; then MODPROBE_MODULES;						fi
