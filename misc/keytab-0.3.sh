...
declare -x HOSTNAME=$(hostname --fqdn)
...
for OPTIONS in $@; do
	...
	--user=*)	declare -x USER_NAME="-U ${OPTIONS#*=}";;
	--pass=*)	declare -x PASSWORD="--password ${OPTIONS#*=}"
    esac
done
...
if [ ${#IP_ADDRESS}	-eq 0 ]; then declare -x IP_ADDRESS="172.16.184.2";				fi
...
for X in "$HOST" "host/$HOST" "host/$HOSTNAME" "RestrictedKrbHost/$HOST" "RestrictedKrbHost/$HOSTNAME" "HOST/$HOSTNAME/$HOSTNAME" "HOST/$HOSTNAME"      \
    ...
    $ST_BIN domain exportkeytab $KEYTAB --realm=$REALM --principal=$PRINCIPAL --ipaddress=$IP_ADDRESS $USER_NAME $PASSWORD
    declare -i RETVAL=$?
