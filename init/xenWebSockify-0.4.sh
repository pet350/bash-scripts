...
XEN_FUNCTIONS="/usr/local/src/xen-scripts.sh"
...
export IF_NAME="lo";	export SOURCE_ADDRESS="$(GET_IP)"
export IF_NAME="br0";	export TARGET_ADDRESS="$(GET_IP)"
...
declare -ig BOL_TARGET_OPTIONS=$TRUE
declare -ig BOL_SSL=$TRUE
...
function do_WEBSOCKIFY()
{
  ...
  if [ $BOL_TARGET_OPTIONS -ne $TRUE ]; then unset TARGET_OPTION; fi
  if [ $BOL_SSL -ne $TRUE ]; then
    unset SSL_OPTION
    unset CERT_OPTION
  else
    export CERT_OPTION="--cert=$CERT_FILE"
  fi
  while [ $INDEX -lt $DOMAIN_COUNT ]; do
    ...
    declare -ag WS_OPT_ARAY=("$DAEMON_OPTION" "$SSL_OPTION" "--web=$WEB_PREFIX" "$TARGET_OPTION" \
                "$CERT_OPTION" "--heartbeat=$VAR_HEARTBEAT" "--log-file=$LOG_FILE" "$VERBOSE" \
...
'restart')
	...
	;;
'--no-config')
	export BOL_TARGET_OPTIONS=$FALSE
	;;
'--with-config')
	export BOL_TARGET_OPTIONS=$TRUE
	;;
'--no-ssl')
	export BOL_SSL=$FALSE
	;;
'--with-ssl')
	export BOL_SSL=$TRUE
	;;
...
export COMMAND="$WS_BIN"
if [ $BOL_COLOR -eq $TRUE ]; then INIT_COLOR_SHORTHAND; fi
if [ $BOL_VERBOSE -eq $TRUE ]; then
  echo -e "$RUN_CMD\tVersion: $VERSION\nCommon Definitions\tVersion: $COMDEF_VERSION"
fi
...
printf "%b" $CC; StoreXenArray; printf "%b" $CN
