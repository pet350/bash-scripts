...
export SUSE_MACHINE="x86_64-suse-linux"
...
if [ ! -f $__KEYTAB ]; then
  #cp --preserve=all $KT_SOURCE_FILE $__KEYTAB_PREFIX
  # Changed From Copy To Symbolic Link To Try To Easy Managing Too Many Files
  ln -s $KT_SOURCE_FILE $__KEYTAB_PREFIX
fi

# Install k5start if it does not exist
if [ ! -f $__EXEC_FILE ]; then
  if [ "$MACHTYPE" != "$SUSE_MACHINE" ]; then
...
if [ $(id -u) -eq 0 ]; then
  __DAEMON_OPTIONS="$__DAEMON_OPTIONS -u $__ADMIN"
else
  __DAEMON_OPTIONS="$__DAEMON_OPTIONS -u $(whoami)@GIGAWARE.LAN"
fi
...
if [ $BOL_START -eq $TRUE ]; then
	if [ ! -f $__PID_FILE ]; then
	   if [ $BOL_QUIET -eq $FALSE ]; then log_daemon_msg "Starting $RUN_CMD"; fi
	   if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "$__EXEC_FILE $__OPTIONS"; fi
