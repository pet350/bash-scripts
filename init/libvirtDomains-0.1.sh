...
export LIBVIRT_PREFIX="$LIBVIRT_PREFIX/qemu"
export LIBVIRT_INIT_PREFIX="$LIBVIRT_PREFIX/init"
...
export VIRSH_LIBVIRT_OPT="-c qemu:///system"
...
declare -ig DOMAIN_COUNT=$(( $($VIRSH_BIN $VIRSH_LIBVIRT_OPT $LIST_OPT | $WC_BIN -l)-3 ))

# Define Global Arrays
declare -ag LIBVIRT_DOMAIN_ARRAY=();
...
function COUNT_DOMAINS()
{
  ...
  RETVAL=$(( $($VIRSH_BIN $VIRSH_LIBVIRT_OPT $LIST_OPT | $WC_BIN -l)-3 ))
  if [ $BOL_DEBUG -eq $TRUE ]; then echo -e "[Debug] Running LIBVIRT Domain Count:\t$RETVAL\n"; fi
...
if [ ! -d $LIBVIRT_INIT_PREFIX ]; then mkdir -p $LIBVIRT_INIT_PREFIX; fi

function STORE_LIBVIRT_DOMAIN_ARRAY()
{
  ...
  COUNT_DOMAINS
  if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Current Running LIBVIRT Domain Count:\t$DOMAIN_COUNT\n"; fi
  while IFS= read line; do
    ...
    for TEMP in $line; do
      ...
      if [ $LINE_COUNT -eq 1 ]; then
        ((INDEX++))
        LIBVIRT_DOMAIN_ARRAY[$((INDEX))]="$TEMP"
        if [ $BOL_DEBUG -eq $TRUE ]; then echo -e "[Debug] TEMP:\t$TEMP\n"; fi
      fi
    done
  done < <( $VIRSH_BIN $VIRSH_LIBVIRT_OPT $LIST_OPT | $TAIL_BIN --lines=$((DOMAIN_COUNT+1)) | $HEAD_BIN --lines=$((DOMAIN_COUNT)) )
  return $INDEX
};

function SHUTDOWN_LIBVIRT_DOMAINS()
{
  declare -i RETVAL=$SUCCESS
  ...
  for LIBVIRT_DOMAIN in ${LIBVIRT_DOMAIN_ARRAY[@]}; do
    if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Executing: $VIRSH_BIN $VIRSH_LIBVIRT_OPT $SHUTDOWN_OPT $LIBVIRT_DOMAIN"; fi
    $VIRSH_BIN $VIRSH_LIBVIRT_OPT $SHUTDOWN_OPT $LIBVIRT_DOMAIN
    export RETVAL=$?
...
function CREATE_LIBVIRT_DOMAIN()
{
  declare -i RETVAL=$SUCCESS
  if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Attempting To Create $DOMAIN"; fi
  if [ $BOL_DEBUG -eq $TRUE ]; then echo -e "[Debug] $VIRSH_BIN $VIRSH_LIBVIRT_OPT $CREATE_OPT $DOMAIN"; fi
  $VIRSH_BIN $VIRSH_LIBVIRT_OPT $CREATE_OPT $DOMAIN
  export RETVAL=$?
...
function CREATE_LIBVIRT_DOMAIN_LOOP()
{
  declare -i RETVAL=$SUCCESS
  declare -i COUNT=0
  ls -1 $LIBVIRT_INIT_PREFIX/*.xml 2>/dev/null >/dev/null
  if [ $? -eq $SUCCESS ]; then
    if [ $BOL_WAIT -eq $TRUE ]; then $SLEEP_BIN $VAR_WAIT; fi
    for DOMAIN in $( $FIND_BIN $LIBVIRT_INIT_PREFIX -name '*.xml' ); do
      BOL_LOOP=$TRUE
      COUNT=0
    ...
    if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "$LIBVIRT_INIT_PREFIX is Empty! Nothing To Do!"; fi
  fi
  return $RETVAL
};
...
if [ $BOL_STOP -eq $TRUE ]; then
  STORE_LIBVIRT_DOMAIN_ARRAY
  SHUTDOWN_LIBVIRT_DOMAINS
  export EXIT_VAL=$?
