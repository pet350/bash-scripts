...
declare -ig BOL_START_DM=$TRUE
declare -ig BOL_VERBOSE=$FALSE
declare -ig BOL_REQ_PARM=$FALSE
...
# Function will Return $SUCCESS if $DISPLAYMANAGER Is Running $FAILURE if it is Not!
function CheckDisplayManager()
{
  declare -i RETVAL=$FAILURE
  ps axho comm| grep $DISPAYMANAGER > /dev/null
  RETVAL=$?
  if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Display Manager $DISPAYMANAGER Status is: $RETVAL"; fi
  return $RETVAL
};
...
function do_START()
{
  ...
  if [ $RETVAL -eq $SUCCESS ]; then
    CheckDisplayManager
    if [ $? -eq $SUCCESS ]; then
      if [ $BOL_START_DM -eq $TRUE ]; then /etc/init.d/$DISPAYMANAGER restart; fi
    else
      if [ $BOL_START_DM -eq $TRUE ]; then /etc/init.d/$DISPAYMANAGER start; fi
    fi
  fi
  return $RETVAL
};
...
function do_HELP()
{
  ...
  printf "%-15s\t\t%-25s\n\n" "--no-dm" "Do NOT Start/Restart Display Manager"
...
if [ $BOL_REQ_PARM -eq $FALSE ]; then
	echo -e "$RUN_CMD Version $VERSION Required Parameter Missing\nUsage: $RUN_CMD { start | stop | restart } --help"
	RETVAL=$FAILURE
fi
