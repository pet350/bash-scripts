...
source /usr/local/scripts/source/ClearOpenVSwitch.sh
...
declare -ig BOL_CLEAR_OVCTL=$TRUE
...
function makeBRIDGE()
{
  ...
  if [ $BOL_BOND -eq $TRUE ]; then
    if [ $BOL_VERBOSE -eq $TRUE ]; then
      echo -e "$OVS_BIN add-bond $BRIDGE_IFACE $BOND_IFACE $NETWORK_IFACE bond_mode=balance-slb"
    fi
    $OVS_BIN add-bond $BRIDGE_IFACE $BOND_IFACE $NETWORK_IFACE bond_mode=balance-slb
    RETVAL=$?
...
if [ $BOL_CLEAR_OVCTL -eq $TRUE ]; then ClearOVCTL; fi
