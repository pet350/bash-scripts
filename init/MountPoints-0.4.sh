...
{
  export BOL_BTRFS=$TRUE
  export BTRFS_MOUNT_OPTIONS="-o defaults,compress=lzo"

  declare -ag DEVICE_ARRAY=(	"/dev/mapper/sql.guest-root.disk" \
  ...
  declare -ag MOUNT_ARRAY=(	"$VM_SQL" "$VM_SQL/var/lib/mysql" "$VM_SQL/var/lib/mysql-keyring" "$VM_SQL/var/lib/mysql-files" "$VM_SQL/var/lib/mysql-upgrade" "$VM_SQL/var/log/mysql" \
...
if [ $BOL_START -eq $TRUE ]; then
	...
	if [ $BOL_VERBOSE -eq $TRUE ]; then LOG_RESULTS; fi
	if [ $BOL_XEN -eq $FALSE ]; then
	    CREATE_VM_PREFIXES		# Create Virtual Machine Container Prefixes
