...
export TEMP_USERNAME="$(whoami)"
export WHOAMI=${TEMP_USERNAME%%@*}
...
export __BASE_DN="cn=users,dc=gigaware,dc=lan"
...
export PRI_SERVER="lxc.gigaware.lan"
export SEC_SERVER="dc.gigaware.lan"

# Function attempts to find Kerberos 5 Server
function FIND_KT_SERVER()
{
  $PING_BIN -c 1 $PRI_SERVER >/dev/null 2>/dev/null 3>/dev/null
  declare -ig PRI_SERVER_STATUS=$?
  $PING_BIN -c 1 $SEC_SERVER >/dev/null 2>/dev/null 3>/dev/null
  declare -ig SEC_SERVER_STATUS=$?

  if [ $PRI_SERVER_STATUS -eq $SUCCESS ]; then
    export KT_SERVER="$PRI_SERVER"
  elif [ $SEC_SERVER_STATUS -eq $SUCCESS ]; then
    export KT_SERVER="$SEC_SERVER"
  else
    echo -e "$RUN_CMD Version $_VER\nCannot find a running IPA Server!\n"
    exit $FAILURE
  fi
  return $SUCCESS
};

# Attempt to find running Kerberos server IF KT_SERVER is not set by Environment
if [ ${#KT_SERVER} -eq 0 ]; then FIND_KT_SERVER; fi

# Where to look for keytab files
...
function GET_LDAP_JPEG()
{
  ...
  for TEMP in $DATA; do
	...
	if [ $BOL_VERBOSE -eq $TRUE ]; then
	  PRINT_DATE_TIME
	  printf "%bFound: %bEmbedded JPEG in LDAP Account! %b\n" $COLOR_LT_BLUE $COLOR_YELLOW $COLOR_NORMAL
	  PRINT_DATE_TIME
	...
	if [ $BOL_VERBOSE -eq $TRUE ]; then
	  PRINT_DATE_TIME
	  printf "%bExecuting: %bchown %s " $COLOR_LT_BLUE $COLOR_YELLOW "$WHOAMI:domain users"
	  printf "%s %s " $AVATAR_FILE $AVATAR_FILE_ICON
	fi
	chown $WHOAMI:"domain users" $AVATAR_FILE $AVATAR_FILE_ICON >/dev/null 2>/dev/null
        RETVAL=$?
	FUNCTION_RETURN=$((FUNCTION_RETURN+RETVAL))
        if [ $BOL_VERBOSE -eq $TRUE ]; then REPORT_STATUS; fi
  ...
  if [ $BOL_FOUND_JPG -eq $FALSE ]; then
    if [ $BOL_VERBOSE -eq $TRUE ]; then
	PRINT_DATE_TIME
        printf "%bNot Found: %bNo Embedded JPEG in LDAP Account! %b\n" $COLOR_LT_BLUE $COLOR_YELLOW $COLOR_NORMAL
    fi
  fi
...
declare -ag HELP_ARRAY=("-------------" "-----------------------------------------------------------------\n" \
	...
	"Same as: --quiet --no-klist --no-whoami --no-wait --ignore --alt restart\n" \
	...
	"-----------------------------------------------------------------\n" "Color" "Enable/Disable ANSI colorized output.\n" \
	"--bw" "Disable color. Black & White text only. (bad formatting)\n" "--color" "Enable color if supported by detected terminal. (default)\n" \
	"--force-color" "Enable color ignoring detected terminal.");
