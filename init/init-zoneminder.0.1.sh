#!/bin/bash
### BEGIN INIT INFO
# Provides:          Starts ZoneMinder Service if Remote MYSQL is Running
...
export MYSQL_ADMIN="$BIN_PREFIX/mysqladmin"
...
function do_START()
{
    ...
    $INIT_PREFIX/$LOCAL_SERVICE_NAME start
    RETVAL=$?
    return $RETVAL
};
...
function do_STOP()
{
    $INIT_PREFIX/$LOCAL_SERVICE_NAME stop
    RETVAL=$?
    return $RETVAL
};
