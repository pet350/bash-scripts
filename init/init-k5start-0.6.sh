...
function GET_LDAP_JPEG()
{
  ...
  if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Executing: $__LDAP_BIN $SEARCH_OPTIONS"; fi
  ...
  for TEMP in $DATA; do
	...
	if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Found Imbeded JPEG in LDAP Account!"; fi
	mv $FILENAME $AVATAR_FILE_TMP 2>/dev/null
	if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Executing: $__CONVERT_BIN -geometry 96x96! $AVATAR_FILE_TMP $AVATAR_FILE_PNG"; fi
	$__CONVERT_BIN -geometry 96x96! $AVATAR_FILE_TMP $AVATAR_FILE_PNG >/dev/null 2>/dev/null
        if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Executing: $__CONVERT_BIN -geometry 96x96! $AVATAR_FILE_TMP $AVATAR_FILE_JPG"; fi
        $__CONVERT_BIN -geometry 96x96! $AVATAR_FILE_TMP $AVATAR_FILE_JPG >/dev/null 2>/dev/null
	#cp $AVATAR_FILE_PNG $AVATAR_FILE >/dev/null 2>/dev/null
        cp $AVATAR_FILE_JPG $AVATAR_FILE >/dev/null 2>/dev/null
...
if [ $BOL_START -eq $TRUE ]; then
	if [ ! -f $__PID_FILE ]; then
	   if [ $BOL_QUIET -eq $FALSE ]; then log_daemon_msg "Starting $RUN_CMD"; printf "\n"; fi
	   if [ $BOL_ALT -eq $TRUE ]; then
		if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Executing: $__ALT_BIN $__ALT_OPTIONS"; fi
		$__ALT_BIN $__ALT_OPTIONS >/dev/null 2>/dev/null
		export RETVAL=$?
	   fi
	   if [ $BOL_K5START -eq $TRUE ]; then
	       if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "Executing: $__EXEC_FILE $__OPTIONS"; fi
