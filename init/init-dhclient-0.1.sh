...
if [ $BOL_STOP -eq $TRUE ]; then
        ...
        start_daemon -p $__PID_FILE $__EXEC_FILE -v -r $__OPTIONS
        RETVAL=$?
fi

if [ $BOL_START -eq $TRUE ]; then
        log_daemon_msg "Starting $RUN_CMD"
        start_daemon -p $__PID_FILE $__EXEC_FILE -v $__OPTIONS
        RETVAL=$?
fi
