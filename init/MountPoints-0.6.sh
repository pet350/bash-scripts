...
function INIT_MOUNT_POINTS()
{
  ...
  export BTRFS_MOUNT_OPTIONS="-o defaults,compress=lzo,autodefrag,clear_cache,noinode_cache,nospace_cache"
...
function INIT_EXT_MOUNT_POINTS()
{
  ...
  declare -ag DEVICE_ARRAY=(	"$MAPPER/xen.data-qemu.disk"  );

  declare -ag MOUNT_ARRAY=(	"/opt/qemu"  );
...
{
  export BOL_BTRFS=$TRUE
  export BTRFS_MOUNT_OPTIONS="-o defaults,compress=lzo,nospace_cache"
