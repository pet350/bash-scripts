...
export _FILE="/etc/xen/pci.conf"

declare -ag _ARRAY_UNKNOWN=()
declare -ag _ARRAY_PCI_DEVICE=()
...
# Function for Storing All PCI ID(s) into Declared Array
function Store_ARRAY_PCI_DEVICE()
{
   INDEX=0
   exec 3<&0
   exec 0<$_FILE
...
if [ ! -f $_FILE ]; then
        echo -e "File $_FILE does not exist!\n"
        _BOL_START=0
        _BOL_STOP=0
        _BOL_VERBOSE=0
        _BOL_LIST_DEVICES=0
