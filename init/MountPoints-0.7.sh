...
# Define Path Prefix String Variables
export BIN_PREFIX="/bin"
export SBIN_PREFIX="/usr/sbin"
export CFG_PREFIX="/etc/xen"

# Define Application Binary String Variables
...
# Define Boolean Variables And Set Thier Default Values
declare -ig BOL_START=$FALSE
declare -ig BOL_STOP=$FALSE
declare -ig BOL_VERBOSE=$FALSE
declare -ig BOL_DESTROY_NBD_DEVICES=$FALSE
declare -ig BOL_FORCE=$FALSE
...
declare -ig BOL_CHECK_MOUNT=$TRUE
declare -ig BOL_UNMOUNT_LVM_MOUNT_POINTS=$TRUE
declare -ig BOL_CREATE_VM_PREFIXES=$FALSE
declare -ig BOL_CREATE_NBD_DEVICES=$FALSE
declare -ig BOL_INIT_LVM_MOUNT_POINTS=$FALSE
declare -ig BOL_INIT_EXT_MOUNT_POINTS=$TRUE
...
export NFS_AUTOMOUNT="$CFG_PREFIX/auto.net"
export COMPRESS_OPTION="zlib"

# Define Virtual Machine Prefixes
export CONTAINER_PREFIX="/opt/ram/container"
...
# Function will create VM Prefixes IF they don't already exist
function CREATE_VM_PREFIXES()
{
  if [ ! -d $VM_WWW ]; then mkdir -p $VM_WWW; fi
  if [ ! -d $VM_SQL ]; then mkdir -p $VM_SQL; fi
  if [ ! -d $VM_UBS ]; then mkdir -p $VM_UBS; fi
...
function INIT_MOUNT_POINTS()
{
  ...
  export BTRFS_MOUNT_OPTIONS="-o defaults,compress=$COMPRESS_OPTION,autodefrag,clear_cache"

  # Define Global Default Mount Points
  declare -ag DEVICE_ARRAY=('UUID=305c0cdc-ecaf-4b4b-a710-7243706ac7ba' \
			    'UUID=87ecaf72-12b9-4287-b1ec-ebc6cdc9934b' 'UUID=87ecaf72-12b9-4287-b1ec-ebc6cdc9934b' \
			    'UUID=34c1d0d1-fef8-42df-9827-22313583ae21' 'UUID=34c1d0d1-fef8-42df-9827-22313583ae21' \
			    'UUID=34c1d0d1-fef8-42df-9827-22313583ae21');
  declare -ag SUBVOL_ARRAY=('@home' '@xen' '@download' '@System.Configs' '@Disk.Images' '@Disk.Images');
  declare -ag MOUNT_ARRAY=('/home/' '/opt/xen' '/opt/download' '/opt/bak' '/opt/images' '/opt/images');
...
function INIT_EXT_MOUNT_POINTS()
{
  ...
  declare -ag DEVICE_ARRAY=("$(blkid -U e3c8c869-6b17-4ff1-9f34-7e94ef91e901)" \
			    "$(blkid -U 804a9fe8-7766-4e3f-8907-a261b920c461)" \
			    "$(blkid -U 28896cda-b06b-4fe8-8a25-187ebf3808b3)");

  declare -ag MOUNT_ARRAY=("/opt/qemu" "/opt/temp" "/opt/build");

  declare -ag FSCK_ARRAY=("/sbin/fsck.ext4" "/sbin/fsck.jfs" "/sbin/xfs_repair");

  REVERSE_ARRAYS

  for BIN in ${FSCK_ARRAY[@]}; do
    ((INDEX++))
    DEVICE="${DEVICE_ARRAY[$((INDEX))]}"
    if [ $BOL_VERBOSE -eq $TRUE ]; then echo -e "$BIN $DEVICE"; fi
    $BIN $DEVICE
    RETVAL=$?
  done
...
{
  export BOL_BTRFS=$TRUE
  export BTRFS_MOUNT_OPTIONS="-o defaults,compress=$COMPRESS_OPTION,nospace_cache"

  declare -ag DEVICE_ARRAY=(	"/dev/mapper/ubuntuserver.guest-root.disk" \
  ...
  declare -ag SUBVOL_ARRAY=(	'@' '@home.bak' '@iso.bak' '@movies.bak' '@mp3drive.bak' '@porn.bak' '@roms.bak' '@storage.bak' '@home.bak');

  declare -ag MOUNT_ARRAY=(	"$VM_UBS" "$VM_UBS/opt/data.bak/home.bak" "$VM_UBS/opt/data.bak/iso.bak" "$VM_UBS/opt/data.bak/movies.bak" "$VM_UBS/opt/data.bak/mp3drive.bak" "$VM_UBS/opt/data.bak/porn.bak" \
...
function MOUNT_LOOP()
{
  ...
  for TEMP_DATA in ${DEVICE_ARRAY[@]}; do
    ...
    $MOUNT_BIN | $GREP_BIN -v "$NFS_AUTOMOUNT" | $GREP_BIN "$TEMP_MOUNT " >/dev/null
    RESULTS=$?
    if [ $BOL_CHECK_MOUNT -eq $FALSE ]; then RESULTS=$FAILURE; fi
    if [ $RESULTS -ne $SUCCESS ]; then
...
'restart')
	...
	export BOL_START=$TRUE
	;;
## Currently The Default For Creating Virtual Machine Prefixes is Disabled
'--enable-vm-prefixes')
	export BOL_CREATE_VM_PREFIXES=$TRUE
	;;
'--disable-vm-prefixes')
	export BOL_CREATE_VM_PREFIXES=$FALSE
	;;
## Curreclty The Default For Creating NBD Devices is Disabled
'--enable-nbd-devices')
	export BOL_CREATE_NBD_DEVICES=$TRUE
	;;
'--disable-nbd-devices')
	export BOL_CREATE_NBD_DEVICES=$FALSE
	;;
## Currently The Drfault For LVM Mount is Disabled
'--enable-lvm-mount')
	export BOL_INIT_LVM_MOUNT_POINTS=$TRUE
	;;
'--disable-lvm-mount')
	export BOL_INIT_LVM_MOUNT_POINTS=$FALSE
	;;
## Currently The Default For EXT Mount is Disabled
'--enable-ext-mount')
	export BOL_INIT_EXT_MOUNT_POINTS=$TRUE
	;;
'--disable-ext-mount')
	export BOL_INIT_EXT_MOUNT_POINTS=$FALSE
	;;
'--enable-check')
...
'-t' | '--test')
	...
	export MOUNT_BIN="$BIN_PREFIX/false"
	export UMOUNT_BIN="$BIN_PREFIX/false"
	;;
'-x' | '--with-xen')
	export BOL_XEN=$TRUE
	;;
...
'-v' | '--verbose')
	...
	;;
'--version')
	echo -e "$RUN_CMD\tVersion: $VERSION\nBy: Peter Talbott\tRevised: 03/25/2020"
	exit $SUCCESS
	;;
-w=* | --wait=*)
        X="${i#*=}"
...
if [ $BOL_FORCE -eq $FALSE ]; then BOL_STOP=$FALSE; fi
...
if [ $BOL_STOP -eq $TRUE ]; then
	...
	if [ $BOL_XEN -eq $FALSE ]; then
	    ...
	    if [ $BOL_UNMOUNT_LVM_MOUNT_POINTS -eq $TRUE ]; then
	    ...
	    if [ $BOL_DESTROY_NBD_DEVICES -eq $TRUE ]; then
	      DESTROY_NBD_DEVICES
	      export RETVAL=$?
	      if [ $BOL_VERBOSE -eq $TRUE ]; then LOG_RESULTS; fi
	    fi
...
if [ $BOL_START -eq $TRUE ]; then
	...
	if [ $BOL_VERBOSE -eq $TRUE ]; then LOG_RESULTS; fi
	if [ $BOL_XEN -eq $FALSE ]; then
	    if [ $BOL_CREATE_VM_PREFIXES -eq $TRUE ]; then CREATE_VM_PREFIXES; fi		# Create Virtual Machine Container Prefixes

	    if [ $BOL_CREATE_NBD_DEVICES -eq $TRUE ]; then
		CREATE_NBD_DEVICES								# Create NBD Devices From Guest LVMs
		export RETVAL=$?
		if [ $BOL_VERBOSE -eq $TRUE ]; then LOG_RESULTS; fi
	    fi

	    if [ $BOL_INIT_LVM_MOUNT_POINTS -eq $TRUE ]; then
	    ...
	    if [ $BOL_INIT_EXT_MOUNT_POINTS -eq $TRUE ]; then
		INIT_EXT_MOUNT_POINTS
	    	MOUNT_LOOP
	    	export RETVAL=$?
	    	if [ $BOL_VERBOSE -eq $TRUE ]; then LOG_RESULTS; fi
