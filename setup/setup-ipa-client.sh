...
export IPA_HOSTNAME="www.gigaware.lan"
export DNS_ZONE="gigaware.lan"
export REV_ZONE="184.16.172.in-addr.arpa."
export IPA_IP_ADDRESS="172.16.184.14"

# Define Binary Options
export RNDC_OPTIONS="-a -b 512"
declare -ag SERVER_OPTIONS=( "--mkhomedir" "--force" "--permit" "--enable-dns-updates" \
 "--hostname=$IPA_HOSTNAME" "--domain=$DNS_ZONE" "--realm=$REALM" "--server=lxc.gigaware.lan" "--force-join" \
 "--ntp-pool=pool.ntp.org" "--ip-address=$IPA_IP_ADDRESS" "--verbose" );
...
export IPA_SERVER_INSTALL_BIN="$USR_PREFIX$SBIN_PREFIX/ipa-client-install"
