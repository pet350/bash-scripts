...
export GLOBAL_STRINGS_VERSION=0.3.2
...
if [ ${#LSB_RELEASE}	-eq 0 ]; then LSB_RELEASE=$(lsb_release -d); LSB_RELEASE=${LSB_RELEASE#Description:*};	export LSB_RELEASE="${LSB_RELEASE:1}";	fi
...
declare -ag CMD_ARRAY=( "arp" "apt" "blkid" "bzcat" "bzip2" "chgrp" "chmod" "chown" "compton" "cut" "curl" "docker" "daemon" 	\
  "du" "false" "fdisk" "ffmpeg" "ffplay"  "free" "fsck" "grep" "gzip" "head" "ifconfig" "insmod" "inotifywait"			\
  ...
  "tar"	"true" "tty" "umount" "unison" "uptime" "virsh" "wc" "wbinfo" "xfs_repair" "xl" "xrandr" "xz"	"xzcat"			);
...
if [ ${#FFOPTS264LO}    -eq 0 ]; then export FFOPTS264LO="-c:v libx264 -vf scale=480x360,fps=fps=29.97 -aspect 16:9 -tune zerolatency -preset ultrafast -c:a libmp3lame -movflags faststart";											fi
...
if [ ${#TTY}		-eq 0 ]  && [ ${#TTY_BIN} -ne 0 ]; then export TTY="$($TTY_BIN)"; 																							fi

case $TTY in
     /dev/pts/* | /dev/ttyS*)	export INTERACTIVE="Yes";;
